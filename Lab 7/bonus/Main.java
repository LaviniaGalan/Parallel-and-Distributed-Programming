...
public class Main {
    ...
    public static Polynomial generatePolynomialFromNumber(String number){
        List<Integer> coefficients = new ArrayList<>();
        for(int i = number.length() - 1; i >= 0; i--)
        {
            coefficients.add(Integer.parseInt(String.valueOf(number.charAt(i))));
        }
        return new Polynomial(coefficients);
    }

    public static void mainProcess(int size){
        Polynomial p = generatePolynomialFromNumber("9999999");
        Polynomial q = generatePolynomialFromNumber("9000999");
